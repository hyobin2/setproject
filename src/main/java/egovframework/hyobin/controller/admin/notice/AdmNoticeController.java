 ...
 */
package egovframework.hyobin.controller.admin.notice;
...
@Controller
@RequestMapping(value = AdmNoticeController.PREFIX)
public class AdmNoticeController {

	public static final String PREFIX = "/adm/notice";
	public static final String B_CODE = GlobalsProperties.getProperty("Bbs.notice"); // DB에 테이블을 하나로써서 B_CODE로 구별하기 위해 필요// Bbs.sponson=B01 (설정을 따로함)
	public static final String FILE_UPLOAD_PATH = GlobalsProperties.getFileProperty("notice.Path"); // DB에 테이블을 하나로써서upload경로를 따로지정//sponson.Path=/upload/editor/images(설정을따로함)
	/** BoardService */
