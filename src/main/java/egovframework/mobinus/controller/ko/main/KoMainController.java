 ...
 */
package egovframework.mobinus.controller.ko.main;
...
@Controller
@RequestMapping(value = KoMainController.PREFIX)
public class KoMainController {
    ...
    public String list(MyMap paramMap, ModelMap model)
		...
		paramMap.put("bCode", GlobalsProperties.getProperty("Bbs.Now"));
		paramMap.put("firstIndex", 0);
		paramMap.put("recordCountPerPage", 6);

		List<Map<String, Object>> nowList = boardService.list(paramMap.getMap());

		if( nowList != null && nowList.size() > 0 ){
			for( int i = 0; i < nowList.size(); i++ ){
				Map<String, Object> tmpListMap = nowList.get(i);
