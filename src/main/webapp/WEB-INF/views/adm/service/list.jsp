...
function fn_write(iIdx) {
	$('#iIdx').val(iIdx);
	$('#baseForm').attr('action', '/adm/service/write.do');
	$('#baseForm').submit();
}
		...
		<input type="hidden" name="pageIndex" id="pageIndex" value="${paramMap.pageIndex }"/>
       	<div class="table_area">
            <div class="">
				...
				<table summary="견적문의관리" class="style5">
				<caption class="hide">견적문의관리</caption>
					<colgroup>
						<col width="5%">
						<col width="">
						<col width="10%">
					...
					<tbody>
						<tr>
							...
							<c:forEach var="result" items="${list}" varStatus="status">
								<tr>
									<td>${result.rnum}</td>
									 <td class="t_left"><a href="#" onclick="fn_write('${util:zeroConvert(result.iIdx)}'); return false;">${result.title }</a></td>
									<td>${result.company}</td>
									<td>
			                        	<c:choose>
			                        		<c:when test="${result.replyYn == 'Y'}">답변</c:when>
