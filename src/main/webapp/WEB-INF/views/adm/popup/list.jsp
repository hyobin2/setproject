...
function fn_write(pIdx) {
	$('#pIdx').val(pIdx);
	$('#baseForm').attr('action', '/adm/popup/write.do');
	$('#baseForm').submit();
}
...
function fn_search_page(){
    ...
    <div class="s_con_area">
       	...
       	<form name="baseForm" id="baseForm" method="get" action="/adm/popup/list.do" >
		<input type="hidden" name="pIdx" id="pIdx" value=""/>
		<input type="hidden" name="pageIndex" id="pageIndex" value="${paramMap.pageIndex }"/>
		<input type="hidden" name="pCode" id="pCode" value="POP01"/>
